...
static void filtered_fwrite(FILE *f, const char *buf, int len, int use_isprint, int trailing_CR_or_NL)
{
...
void rwrite(enum logcode code, const char *buf, int len, int is_utf8)
{
	...
	if (ic != (iconv_t)-1) {
		...
		while (inbuf.len) {
			...
			if (outbuf.len) {
                                if ( trailing_CR_or_NL ) {
                                    filtered_fwrite(f, convbuf, outbuf.len - 1, 0, trailing_CR_or_NL);
                                } else {
                                    filtered_fwrite(f, convbuf, outbuf.len, 0, trailing_CR_or_NL);
                                }
				outbuf.len = 0;
			}
