	...
	if (total) {
		...
		for (j = 0; j < 5; j++) {
			if (counts[j]) {
				...
				if (tmplen >= (int)sizeof(buf) - len - 2) {
					break;
				}
				len += tmplen;
				pre = ", ";
...
static int do_recv(int f_in, int f_out, char *local_name)
{
	...
	if (pid == 0) {
		...
		send_msgs_to_gen = am_server;
                ...
                bpc_am_generator(am_generator, pid);
...
# endif
	...
	SIGACTION(SIGXFSZ, SIG_IGN);
        ...
        {
            /*
             * BackupPC initialization
             */
            extern char *bpc_top_dir;
            extern char *bpc_host_name;
            extern char *bpc_share_name;
            extern int bpc_backup_num;
            extern int bpc_backup_compress;
            extern int bpc_backup_prev_num;
